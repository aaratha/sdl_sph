...
bool Render_Init(RenderState *state,
  ...
  if (vertexShader == NULL) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Couldn't create vertex shader: %s", SDL_GetError());
    return false;
  }
  ...
  SDL_GPUShader *fragmentShader = SDL_CreateGPUShader(device, &fragShaderCreateInfo);
  SDL_free(fragCode);
  if (fragmentShader == NULL) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Couldn't create fragment shader: %s", SDL_GetError());
    SDL_ReleaseGPUShader(device, vertexShader);
    return false;
  }
  ...
  if (pipeline == NULL) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Couldn't create graphics pipeline: %s", SDL_GetError());
    SDL_ReleaseGPUShader(device, vertexShader);
