...
SDL_AppResult SDL_AppInit(
  ...
  bool useMSLShaders = (driverName != NULL && SDL_strcmp(driverName, "metal") == 0);
  ...
  SDL_GPUComputePipeline *computePipeline = SDL_CreateGPUComputePipeline(device, &computeCreateInfo);
  if (computePipeline == NULL) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Couldn't create compute pipeline: %s", SDL_GetError());
    SDL_free(compCode);
    return SDL_APP_FAILURE;
  }
  ...
  if (!Render_Init(&render, device, shaderFormat, vertexShaderPath, fragmentShaderPath)) {
  ...
  const int numParticles = 1024; // multiple of 64 to match compute threadgroup dispatch
  ...
  if (xCurr == NULL || yCurr == NULL || xPrev == NULL || yPrev == NULL || mass == NULL || density == NULL) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Couldn't allocate particle data buffers");
    SDL_free(xCurr);
    SDL_free(yCurr);
    SDL_free(xPrev);
    SDL_free(yPrev);
  ...
  for (int i = 0; i < numParticles; i++) {
    ...
    float velX = ((float)((rand() % 200) - 100) / 100.0f) / halfWidth * 2.0f;   // small NDC-scale drift
    float velY = ((float)((rand() % 200) - 100) / 100.0f) / halfHeight * 2.0f;
  ...
  if (xCurrBuffer == NULL || yCurrBuffer == NULL || xPrevBuffer == NULL || yPrevBuffer == NULL ||
      massBuffer == NULL || densityBuffer == NULL) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Couldn't create particle buffers: %s", SDL_GetError());
  ...
  SDL_GPUTransferBuffer *txXCurr = SDL_CreateGPUTransferBuffer(device, &(SDL_GPUTransferBufferCreateInfo){
      .usage = SDL_GPU_TRANSFERBUFFERUSAGE_UPLOAD,
      .size = floatBufferSize});
  SDL_GPUTransferBuffer *txYCurr = SDL_CreateGPUTransferBuffer(device, &(SDL_GPUTransferBufferCreateInfo){
      .usage = SDL_GPU_TRANSFERBUFFERUSAGE_UPLOAD,
      .size = floatBufferSize});
  SDL_GPUTransferBuffer *txXPrev = SDL_CreateGPUTransferBuffer(device, &(SDL_GPUTransferBufferCreateInfo){
      .usage = SDL_GPU_TRANSFERBUFFERUSAGE_UPLOAD,
      .size = floatBufferSize});
  SDL_GPUTransferBuffer *txYPrev = SDL_CreateGPUTransferBuffer(device, &(SDL_GPUTransferBufferCreateInfo){
      .usage = SDL_GPU_TRANSFERBUFFERUSAGE_UPLOAD,
      .size = floatBufferSize});
  SDL_GPUTransferBuffer *txMass = SDL_CreateGPUTransferBuffer(device, &(SDL_GPUTransferBufferCreateInfo){
      .usage = SDL_GPU_TRANSFERBUFFERUSAGE_UPLOAD,
      .size = floatBufferSize});
  SDL_GPUTransferBuffer *txDensity = SDL_CreateGPUTransferBuffer(device, &(SDL_GPUTransferBufferCreateInfo){
      .usage = SDL_GPU_TRANSFERBUFFERUSAGE_UPLOAD,
      .size = floatBufferSize});

  if (txXCurr == NULL || txYCurr == NULL || txXPrev == NULL || txYPrev == NULL || txMass == NULL || txDensity == NULL) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Couldn't create transfer buffers: %s", SDL_GetError());
  ...
  struct {
    ...
    SDL_GPUBuffer *gpuBuf;
      ...
      {txXCurr, xCurr, xCurrBuffer},
      {txYCurr, yCurr, yCurrBuffer},
      {txXPrev, xPrev, xPrevBuffer},
      {txYPrev, yPrev, yPrevBuffer},
      {txMass, mass, massBuffer},
      {txDensity, density, densityBuffer},
  };
  ...
  for (size_t i = 0; i < SDL_arraysize(uploads); i++) {
    ...
    if (mapped == NULL) {
      SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Couldn't map transfer buffer: %s", SDL_GetError());
      mappingFailed = true;
    } else {
  ...
  if (uploadCmdBuf == NULL) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Couldn't acquire command buffer for upload: %s", SDL_GetError());
    SDL_ReleaseGPUTransferBuffer(device, txXCurr);
    SDL_ReleaseGPUTransferBuffer(device, txYCurr);
    SDL_ReleaseGPUTransferBuffer(device, txXPrev);
    SDL_ReleaseGPUTransferBuffer(device, txYPrev);
  ...
  if (copyPass == NULL) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Couldn't begin copy pass: %s", SDL_GetError());
  ...
  for (size_t i = 0; i < SDL_arraysize(uploads); i++) {
    SDL_GPUTransferBufferLocation src = {
        .transfer_buffer = uploads[i].tx,
        .offset = 0};
    SDL_GPUBufferRegion dst = {
        .buffer = uploads[i].gpuBuf,
        .offset = 0,
        .size = floatBufferSize};
...
SDL_AppResult SDL_AppIterate(void *appState) {
  ...
  SDL_GPUComputePass *computePass = SDL_BeginGPUComputePass(cmdBuf, NULL, 0, rwBindings, SDL_arraysize(rwBindings));
  if (computePass == NULL) {
  ...
  SDL_GPUBuffer *computeBuffers[] = {
      ...
      context->yPrevBuffer,
      context->massBuffer,
      context->densityBuffer};
  SDL_BindGPUComputeStorageBuffers(computePass, 0, computeBuffers, SDL_arraysize(computeBuffers));
  SDL_DispatchGPUCompute(computePass, groupCountX, 1, 1);
  SDL_EndGPUComputePass(computePass);

  if (!Render_Draw(&context->render, cmdBuf, context->window,
                   context->xCurrBuffer, context->yCurrBuffer, context->numParticles)) {
    return SDL_APP_FAILURE;
  }
